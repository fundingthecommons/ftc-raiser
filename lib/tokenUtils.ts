...
const getAlchemyNetwork = (chainId: number) => {
    return chainIdToAlchemyNetwork[chainId as keyof typeof chainIdToAlchemyNetwork]
}

export const fetchTokenBalances = async (address: string, chainId: number): Promise<TokenBalance[]> => {
    const settings = {
...
export const fetchTokenMetadata = async (tokens: TokenBalance[], chainId: number) => {
    const settings = {
        apiKey: ALCHEMY_API_KEY,
        network: getAlchemyNetwork(chainId),
    }
