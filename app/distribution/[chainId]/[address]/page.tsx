...
export default function DistributionPage() {
    ...
    const {splitMetadata, isLoading} = useSplitMetadata(Number(chainId), address)
        ...
        <div className="container mx-auto p-4">
            <Card>
                ...
                <CardContent className="space-y-6">
                    ...
                    <div>
                        <h3 className="text-lg font-semibold mb-2">Distribution Incentive</h3>
                            ...
                            {splitMetadata.recipients.map((recipient) => (
                                <div key={recipient.recipient.address} className="space-y-2">
                                    <div className="flex justify-between items-center">
                                        <span className="font-medium">{recipient.recipient.address}</span>
                                        <span>{recipient.percentAllocation}%</span>
                                    </div>
                                    <Progress value={parseFloat(String(recipient.percentAllocation))}/>
                                </div>
                            ))}
                        </div>
                    </div>
